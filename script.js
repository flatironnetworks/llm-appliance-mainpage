...
if (contactForm) {
    contactForm.addEventListener('submit', async (e) => {
        ...
        try {
            // #region agent log
            fetch('http://127.0.0.1:7246/ingest/ba8c2bfc-06ff-4205-87a6-f7e3f32dcdab',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'script.js:100',message:'Before fetch request',data:{workerUrl:workerUrl,hasData:!!data},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B,C'})}).catch(()=>{});
            // #endregion
            ...
            // #region agent log
            fetch('http://127.0.0.1:7246/ingest/ba8c2bfc-06ff-4205-87a6-f7e3f32dcdab',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'script.js:107',message:'After fetch - got response',data:{status:response.status,statusText:response.statusText,hasCorsHeader:response.headers.get('Access-Control-Allow-Origin')},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B,C'})}).catch(()=>{});
            // #endregion
            ...
            // #region agent log
            fetch('http://127.0.0.1:7246/ingest/ba8c2bfc-06ff-4205-87a6-f7e3f32dcdab',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'script.js:125',message:'Fetch catch block',data:{errorName:error.name,errorMessage:error.message},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B,C,D'})}).catch(()=>{});
            // #endregion
